...
//   m_Reader->SetFileName( filename );
  ...
  if ( m_InputImage->GetNumberOfComponentsPerPixel()<3)
    itkExceptionMacro(<<"Invalid input Image, must have at least 3 channels (only "<<m_InputImage->GetNumberOfComponentsPerPixel()<<" in the selected image)");
// 
...
::ErasePolygon(const IndexType & index)
{
  ...
  if ( done == true )
  {
    ...
    itkExceptionMacro(<<"No polygon found.");
}
...
::Classification()
{
  ...
  if ( m_UseSmoothing==true )
    m_Classifier->SetInput(m_MeanShiftFilter->GetOutput());
  else if (m_WhichImage == EXTRACT)
    m_Classifier->SetInput(m_ExtractedImage);
  else
    m_Classifier->SetInput(m_InputImage);
...
// Extract image to sample list :
  ...
  if ( m_UseSmoothing==true )
  {
    ...
    it = itk::ImageRegionIteratorWithIndex<ImageType>(m_MeanShiftFilter->GetOutput(), m_MeanShiftFilter->GetOutput()->GetLargestPossibleRegion());
  }
  else
    it = itk::ImageRegionIteratorWithIndex<ImageType>(m_ExtractedImage, m_ExtractedImage->GetLargestPossibleRegion());
...
::ComputeReferencePixel()
{
  ...
  {
    m_MeanShiftFilter->Update();
    itIm = itk::ImageRegionIteratorWithIndex<ImageType>(m_MeanShiftFilter->GetOutput(), m_MeanShiftFilter->GetOutput()->GetLargestPossibleRegion());
  }
  else
    itIm = itk::ImageRegionIteratorWithIndex<ImageType>(m_ExtractedImage, m_ExtractedImage->GetLargestPossibleRegion());
...
::ComputeSpectralAngle()
{
  ...
  if ( m_WhichImage == FULL_IMAGE)
    ...
    m_SpectralAngleFilter->SetInput( m_ExtractedImage );
...
{
  if ( m_WhichImage == FULL_IMAGE)
    m_MeanShiftFilter->SetInput( m_InputImage );
  else
    m_MeanShiftFilter->SetInput(m_ExtractedImage);
...
::PrepareOutput()
{
  ...
  if (m_WhichImage == EXTRACT)
    m_PersistentVectorizationFilter->Update();
  else
  {
