...
public:
  ...
  virtual void draw()
  {
    ...
    if (m_PointList.size()==4 && m_BoundingBox.size()==4 )
    {
      ...
      int maxSize = std::min(static_cast<int>(static_cast<double>(w)*0.75),static_cast<int>(static_cast<double>(h)*0.75));
      ...
      fl_color(FL_WHITE);
      fl_polygon(intVect[0][0], intVect[0][1], intVect[1][0], intVect[1][1], intVect[2][0], intVect[2][1], intVect[3][0], intVect[3][1] );
      ...
      if ( m_ExtentList.size()!=0 )
      {
        ...
        intVectExtent[0][0] = static_cast<int>(static_cast<double>(intVectExtent[0][0]-m_BoundingBox[0])*r)+xfinal;
        intVectExtent[0][1] = static_cast<int>(static_cast<double>(intVectExtent[0][1]-m_BoundingBox[1])*r)+yfinal;
        intVectExtent[1][0] = static_cast<int>(static_cast<double>(intVectExtent[1][0]-m_BoundingBox[0])*r)+xfinal;
        intVectExtent[1][1] = static_cast<int>(static_cast<double>(intVectExtent[1][1]-m_BoundingBox[1])*r)+yfinal;
        intVectExtent[2][0] = static_cast<int>(static_cast<double>(intVectExtent[2][0]-m_BoundingBox[0])*r)+xfinal;
        intVectExtent[2][1] = static_cast<int>(static_cast<double>(intVectExtent[2][1]-m_BoundingBox[1])*r)+yfinal;
        intVectExtent[3][0] = static_cast<int>(static_cast<double>(intVectExtent[3][0]-m_BoundingBox[0])*r)+xfinal;
        intVectExtent[3][1] = static_cast<int>(static_cast<double>(intVectExtent[3][1]-m_BoundingBox[1])*r)+yfinal;
        ...
        fl_color(FL_GREEN);
        fl_line(intVectExtent[0][0], intVectExtent[0][1], intVectExtent[1][0], intVectExtent[1][1], intVectExtent[2][0], intVectExtent[2][1]);
        fl_line(intVectExtent[2][0], intVectExtent[2][1], intVectExtent[3][0], intVectExtent[3][1], intVectExtent[0][0], intVectExtent[0][1]);
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                if ( std::abs(x-m_PrintedBoundingBoxPointList[0][0]) <= 5 && std::abs(y-m_PrintedBoundingBoxPointList[0][1]) <= 5 )
                {
                  oss.str("");
                  oss<<"("<<m_PointList[m_XBoundingBoxIdList[0]][0]<<" , "<<m_PointList[m_YBoundingBoxIdList[0]][1]<<")"<<std::endl;
                  oss<<"("<<m_LongLatList[m_XBoundingBoxIdList[0]][0]<<" , "<<m_LongLatList[m_YBoundingBoxIdList[0]][1]<<")";
                  m_Tooltip->enter_area(this, 0, 0, 100, 100, oss.str().c_str());
                }
                else
                {
                  if ( std::abs(x-m_PrintedBoundingBoxPointList[1][0]) <= 5 && std::abs(y-m_PrintedBoundingBoxPointList[1][1]) <= 5 )
                  {
                    oss.str("");
                    oss<<"("<<m_PointList[m_XBoundingBoxIdList[1]][0]<<" , "<<m_PointList[m_YBoundingBoxIdList[0]][1]<<")"<<std::endl;
                    oss<<"("<<m_LongLatList[m_XBoundingBoxIdList[1]][0]<<" , "<<m_LongLatList[m_YBoundingBoxIdList[0]][1]<<")";
                    m_Tooltip->enter_area(this, 0, 0, 100, 100, oss.str().c_str());
                  }
                  else
                  {
                    if ( std::abs(x-m_PrintedBoundingBoxPointList[2][0]) <= 5 && std::abs(y-m_PrintedBoundingBoxPointList[2][1]) <= 5 )
                    {
                      oss.str("");
                      oss<<"("<<m_PointList[m_XBoundingBoxIdList[1]][0]<<" , "<<m_PointList[m_YBoundingBoxIdList[1]][1]<<")"<<std::endl;
                      oss<<"("<<m_LongLatList[m_XBoundingBoxIdList[1]][0]<<" , "<<m_LongLatList[m_YBoundingBoxIdList[1]][1]<<")";
                      m_Tooltip->enter_area(this, 0, 0, 100, 100, oss.str().c_str());
                    }
                    else
                    {
                      if ( std::abs(x-m_PrintedBoundingBoxPointList[3][0]) <= 5 && std::abs(y-m_PrintedBoundingBoxPointList[3][1]) <= 5 )
                      {
                        oss.str("");
                        oss<<"("<<m_PointList[m_XBoundingBoxIdList[0]][0]<<" , "<<m_PointList[m_YBoundingBoxIdList[1]][1]<<")"<<std::endl;
                        oss<<"("<<m_LongLatList[m_XBoundingBoxIdList[0]][0]<<" , "<<m_LongLatList[m_YBoundingBoxIdList[1]][1]<<")";
                        m_Tooltip->enter_area(this, 0, 0, 100, 100, oss.str().c_str());
                      }
                      else
                      {
