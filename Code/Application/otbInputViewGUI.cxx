...
::BuildInputInterface()
{
  ...
  if (m_Model.IsNotNull() && m_Controller != NULL)
    ...
    for (it_in = lInputDataMap.begin(); it_in != lInputDataMap.end(); it_in++)
      ...
      for (std::vector<std::string>::const_iterator instIt = moduleInstances.begin();
           instIt != moduleInstances.end();
           instIt++)
        {
...
::RunProcess1(void *)
{
  while (m_Alive)
    ...
    for (InputViewComponentMapType::const_iterator mIt = m_InputViewComponentMap.begin();
         mIt != m_InputViewComponentMap.end();
         ++mIt)
      {
